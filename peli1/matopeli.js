...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkCollision(snake1, snake2) {
        for (let i = 1; i < snake1.length; i++) {
            if (snake1[0].x === snake1[i].x && snake1[0].y === snake1[i].y) return true; // Mato 1 törmää itseensä
        }
        for (let i = 1; i < snake2.length; i++) {
            if (snake2[0].x === snake2[i].x && snake2[0].y === snake2[i].y) return true; // Mato 2 törmää itseensä
        }
